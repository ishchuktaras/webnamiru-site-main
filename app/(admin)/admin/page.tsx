...
export default async function AdminDashboardPage() {
  ...
  const session = await auth(); // Získáme session pro podmíněné zobrazení
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
        ...
        />
        <StatCard
          title="Prům. hodnocení"
          value={stats.averageRating}
          icon={Star}
          color="bg-yellow-500"
        />

        {/* ▼▼▼ KARTA SE STATISTIKOU UŽIVATELŮ (pouze pro SuperAdmina) ▼▼▼ */}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {/* ▼▼▼ AKČNÍ KARTA PRO SPRÁVU UŽIVATELŮ (pouze pro SuperAdmina) ▼▼▼ */}
        {session?.user?.role === "SUPERADMIN" && (
          <ActionCard
            title="Správa Uživatelů"
            description="Přidávejte nové členy týmu, upravujte jejich role a spravujte přístupy do systému."
            href="/admin/users"
            buttonText="Spravovat uživatele"
            icon={Users}
          />
        )}
